...
import static dekusan.DekuSan.ErrorCode.CANCELED;
import static dekusan.DekuSan.ErrorCode.NONE;
import static dekusan.DekuSan.RESULT_ERROR;

public class SignRequestHelper implements Parcelable {
    private Request request;

    private static final Set<String> authorities = new HashSet<String>() {{
        add(DekuSan.ACTION_SIGN_TRANSACTION);
    ...
    public SignRequestHelper(Intent intent, Callback callback) {
        ...
        switch (action) {
            case DekuSan.ACTION_SIGN_MESSAGE: {
    ...
    private static boolean isSignUri(Uri uri) {
        return uri != null && "dekusan".equals(uri.getScheme())
